...
int yylex ()
{
    ...
    {
		yyinStr--;
		/* %lf, not the upstream %f: YYSTYPE is double here, because the
		   pspdev fork's float API was reverted to match rrootage's headers.
		   With %f, sscanf writes 4 bytes through an 8-byte lvalue and every
		   numeric literal in every BulletML formula decodes as garbage -- a
		   denormal reading as 0. Bullets then get speed 0 and sit motionless
		   where they spawned. */
